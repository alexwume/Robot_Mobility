...
for iter = 1:max_iter-1
    ...
    % Motor model
    %add an output "count"
    [F_actual, M_actual, rpm_motor_dot,flag] = motor_model(F, M, current_state.rpm, params,flag);
    
    
    % Get the change in state from the quadrotor dynamics
    timeint = time_vec(iter:iter+1);
    [tsave, xsave] = ode45(@(t,s) dynamics(params, state, F_actual, M_actual, rpm_motor_dot), timeint, state);
    state    = xsave(end, :)';
